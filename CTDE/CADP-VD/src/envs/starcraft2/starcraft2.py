...
class StarCraft2Env(MultiAgentEnv):
    ...
    def check_bounds(self, x , y):
        """Whether a point is whithin the map bounds."""
    ...
    def get_surrounding_height(self, unit):
        ...
        points = self.get_surrounding_points(unit, includ_self=True)
        vals = [
    ...
    def get_obs_agent(self, agent_id):
        """
        Returns obs for agent_id. The obs is composed of:

        - agent movement features (where it can move to, height info and pathing grid).
        - enemy features (available_to_attack, health, relative_x, relatiave_y, shield,
            unit_type).
        - ally features (visible, distance, relative_x, relative_y, shield, unit_type).
        - agent unit features (health, shield, unit_type).


        All of this info is flattend and concatenated into a list, in the aforementioned
        order. To know the sizes of each of the features insied the final list of features,
        take a look at the functions ``get_move_feats_size()``,
        ``get_obs_enemy_feats_size()``, ``get_obs_ally_feats_size()`` and
        ``get_obs_own_feats_size()``.


        The size of the obs vec may very, depending on the env configuration and type of 
        units present in the map. For instance, non-Protoss units will not have shields,
        movement features may or may not include terrain height and pathing grid, unit_type
        is not included if there is only one type of unit in the map etc.)

        NOTE: Agents should have access only to their local obs during decentralized 
        execution.
        """
    ...
    def get_state_dict(self):
        """
        Returns the global state as a dict.

        - allies: numpy array containing agents and their attributes.
        - enemies: numpy array containing enemies and their attributes.
        - last_action: numpy array of previous acts for each agent.
        - timestep: current no. of steps devided by total no. of steps.

        NOTE: This function not be used during decnetralized execution.
        """

        # num of features equals the num of attributes names.
        ...
        for al_id, al_unit in self.agents.items():
            if al_unit.health > 0:
                ...
                ally_state[al_id, 3] = (y - center_y) / self.max_distance_y # relatvie Y
        ...
        for e_id, e_unit in self.enemies.items():
            if e_unit.health > 0:
                ...
                enemy_state[e_id, 2] = (y - center_y) / self.max_distance_y # relatvie Y
    ...
    def get_obs_enemy_feats_size(self):
        """
        Returns the dimensions of the matrix containing enemy features. Size is 
        n_enemies x n_features.
        """
    ...
    def get_obs_own_feats_size(self):
        """Returns the size of the vec containing the agent's own features."""
    ...
    def get_obs_move_feats_size(self):
        """Returns the size of the vec containing the agent's movement-related features."""
    ...
    def get_visibility_matrix(self):
        """
        Returns a boolean numpy array of dims (n_agents, n_agents + n_enemies)
    ...
    def init_units(self):
        ...
        while True:
            # Somethimes not all units have yet been created by SC2
            self.agents = {}
            self.enemies = {}
            ...
            except (protocol.ProtocalError, protocol.ConnectionError):
                self.full_restart()
                self.reset()
    ...
    def update_units(self):
        """Update units after an env step. This function assumes that self._obs is up-to-date."""
    ...
    def _init_ally_unit_types(self, min_unit_type):
        """Initialize ally unit types. Should be called once from 
        the init_units function.
        """
