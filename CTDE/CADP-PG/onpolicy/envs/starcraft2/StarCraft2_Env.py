...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_surrounding_pathing(self, unit):
        ...
        points = self.get_surrounding_points(
            unit, include_self=False
        )
        vals = [
            self.pathing_grid[x, y]
    ...
    def get_obs_agent(self, agent_id):
        ...
        The obs is composed of:
        
            - agent movement features (where it can move to, height ifno and pathing
                grid).
        ...
        All of this info is flattened and concatenated into a list, in the aformentioned
            order. To know the sizes of each of the features inside the final list of features,
            take a look at the functions ``get_obs_move_feats_size()``,
            ``get_obs_enemy_feats_size()``, ``get_obs_ally_feats_size()`` and
            ``get_obs_own_feats_size()``.


        The size of the obs vec may vary, depending on the env configuration and type of units
            present in the map. For instance, non-Protoss units will not have shields, movement
            features may or may not include terrain height and pathing grid, unit_type is not
            included if there is only one type of unit in the map etc.).


        NOTE: Agents should have access only to their local obs during decentralized execution.
        """
        ...
        # otherwise daed, return all zeros
        if unit.health > 0:
            x = unit.pos.x
            y = unit.pos.y
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (dist < sight_range and e_unit.health > 0):
                    ...
                    enemy_feats[e_id, 0] = avail_actions[self.n_actions_no_attack + e_id]   # avail
        ...
        if self.debug:
            logging.debug(
                "Obs Agent: {}".format_map(agent_id).center(60, "-")
            )
            logging.debug(
    ...
    def get_state(self, agent_id=-1):
        ...
        or (not self.use_mustalive):    # or else all zeros
            ...
            for al_id, al_unit in self.agents.items():
                if al_unit.health > 0:
                    ...
                    if self.add_center_xy:
                        ...
                        # center
                        ally_state[al_id, ind + x] = (al_y - center_y) / self.max_distance_y
            ...
            for e_id, e_unit in self.enemies.items():
                if e_unit.health > 0:
                    ...
                    if unit.health > 0:
                        ...
                        if self.add_enemy_action_state:
                            enemy_state[e_id, ind] = avail_actions[self.n_actions_no_attack + e_id]
        ...
        if self.add_local_obs:
            state = np.append(
                state,
                self.get_obs_agent(agent_id).flatten()
            )
    ...
    def get_state_agent(self, agent_id):
        ...
        The obs is composed of:
            ...
            - agent movement features (where it can move eto, height info and pathing grid).
            - enemy features (available_to_attack, health, relative_x, relative_y, shield,
                unit_type).
            - ally features (visible, distance, relative_x, relative_y, shield, unit_type).
            - agent unit features (health, shield, unit_type).


        All of this info is flattened and concatenated into a list, in the aforementioned
            order. To know the sizes of each of the features inside the final list of features,
            take a look at the functions ``get_obs_move_feats_size()``.
            ``get_obs_enemy_feats_size()``, ``get_obs_ally_feats_size()`` and
            ``get_obs_own_feats_size()``.
        ...
        The size of obs vec may vary, depending on the env configuration and type of units present
            in the map. For instance, non-Porotoss units will not have shields, movement features
            may or may not include terrain height and pathing grid, unit_type is not included if there
            is only one type of unit in the map etc.
        ...
        if (self.use_mustalive and unit.health > 0) \
        or (not self.use_mustalive):    # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if e_unit.health > 0:
                    ...
                    if self.add_center_xy:
                        ...
                        enemy_feats[e_id, ind + 1] = (e_y - center_y) / self.max_distance_y # cetner Y
    ...
    def get_state_own_feats_size(self):
        """
        Returns the size of the vector containing the agent's own features.
        """
        own_feats = 4 + self.unit_type_bits
    ...
    def get_visibility_matrix(self):
        ...
        (n_agents, n_agents + n_enemies) indicating which units are visible to each agent.
        """
        arr = np.zeros(
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        """
        unit = self.get_unit_type_id(agent_id)
        ...
        if unit.health > 0:
            ...
            target_imtes = self.enemies.items()
            ...
            for t_id, t_unit in target_imtes:
                if t_unit.health > 0:
                    dist = self.distance(
                        unit.pos.x,
                        unit.pos.y,
    ...
    def _init_ally_unit_types(self, min_unit_type):
        """
        Initialize ally unit types.
        Should be called once from the init_units function.
        """
