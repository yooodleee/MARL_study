...
class SubprocVecEnv(ShareVecEnv):
    ...
    def __init__(
        ...
        for p in self.ps:
            p.daemon = True # if the main process crashes, should not cause things to hang
            p.start()
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def __init__(
        ...
        for p in self.ps:
            p.daemon = True # if the main process crashes, should not things to hang
            p.start()
    ...
    def step_wait(self):
        ...
        obs, share_obs, rews, dones, \
        infos, available_actions = zip(*results)
    ...
    def reset(self):
        ...
        obs, share_obs, avialable_actions = zip(*results)

        return np.stack(obs), np.stack(share_obs), \
            np.stack(avialable_actions)
...
class ChooseSubprocVecEnv(ShareVecEnv):
    ...
    def reset(self, reset_choose):
        for remote, choose in zip(self.remo, reset_choose):
            remote.send(('reset', choose))
...
class DummyVecEnv(ShareVecEnv):
    ...
    def render(self, mode='human'):
        if mode == 'rgb_array':
            ...
            return np.array(
                [env.render(mode=mode) for env in self.envs]
            )
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        obs, share_obs, rews, dones, \
        infos, available_actions = map(np.array, zip(*results))
        ...
        for (i, done) in enumerate(dones):
            if 'bool' in done.__class__.__name__:
                if done:
                    obs[i], \
                    share_obs[i], \
                    available_actions[i] = self.envs[i].reset()
