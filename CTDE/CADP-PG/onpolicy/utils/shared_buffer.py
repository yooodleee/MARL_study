...
class SharedReplayBuffer(object):
    ...
    def chooseinsert(
        ...
        """
        Insert data into the buffer. This insert function is used specifically for
            Handbi, which is turn based.
        ...
        ----------------------
            ...
            masks: (np.ndarray)
                denotes whether the env has terminated or not.
            bad_masks: (np.ndarray)
                denotes indicate whether true terminal state or due to episode
                limit.
            active_masks: (np.ndarray)
                denotes whether an agent is active or dead in the env.
            available_actions: (np.ndarray)
    ...
    def recurrent_generator(
        ...
        ----------------
            ...
            data_chunk_length: (int)
                length of seq chunks with which to train RNN.
        """
        ...
        if len(self.share_obs.shape) > 4:
            share_obs = self.share_obs[:-1].transpose(1, 2, 0, 3, 4, 5).reshape(-1, *self.share_obs[3:])
        ...
        rnn_states = self.rnn_states[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.rnn_states[3:])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                # size [T+1 N M Dim]-->[T N M Dim]-->[N,M,T,Dim]-->[L,Dim]
                ...
                active_masks_batch.append(masks[ind:ind + data_chunk_length])
                old_action_log_probs_batch.append(action_log_probs[ind:ind + data_chunk_length])
                adv_targ.append(advantages[ind:ind + data_chunk_length])
                # size [T+1 N M Dim]-->[T N M Dim]-->[N*M*T,Dim]-->[1,Dim]
