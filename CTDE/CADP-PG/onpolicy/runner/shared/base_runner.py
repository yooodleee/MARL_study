...
class Runner(object):
    ...
    --------------
        config: (dict)
            Config dict containing params for training.
    """
    ...
    def save(self):
        ...
        if self.trainer._use_valuenorm:
            policy_norm = self.trainer.value_normalizer
            torch.save(
                policy_norm.state_dict(),
                str(self.save_dir) + "/vnorm.pt"
            )
