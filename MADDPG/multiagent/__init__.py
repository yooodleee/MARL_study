...
register(
    ...
    # FIXME(cathywu) currently has to be exactly max_path_length parameters
    # in rllab run script.
...
register(
    ...
    max_episode_sptes=100,
)


warnings.warn(
    "This code base is no longer maintained, and is not expected to be maintained agian in the future. \n"
    ...
    "support for the PettingZoo API, support for current versions of Python, numerous bug fixees, \n"
...
if os.getenv('SUPPRESS_MA_PROMPT') != 1:
    input(
        "Please read the raised warining, then press Enter to continue... "
        "(to suppress this prompt, please set the environment variable `SUPPRESS_MA_PROMPT=1`)\n"
    )